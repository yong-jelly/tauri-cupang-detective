...
import { Loader2, Pause, CheckCircle, AlertCircle, Clock, RefreshCw, FastForward, AlertTriangle, Database, Calendar } from "lucide-react";
...
interface CollectionStats {
  lastPayment: LastNaverPaymentInfo | null;
  totalCount: number;
...
export const NaverTransactionCollector = ({ account }: NaverTransactionCollectorProps) => {
  ...
  useEffect(() => {
    const loadCollectionStats = async () => {
      ...
      try {
        // 마지막 결제 정보 조회
        const lastPayment = await invoke<LastNaverPaymentInfo | null>("get_last_naver_payment", {
          userId: account.id,
        });

        // 테이블 통계 조회
        const tableStats = await invoke<TableStat[]>("get_table_stats");
        const paymentTable = tableStats.find(t => t.name === "tbl_naver_payment");
  ...
  const truncateTables = async () => {
    try {
      await invoke("truncate_table", { tableName: "tbl_naver_payment_item" });
      await invoke("truncate_table", { tableName: "tbl_naver_payment" });
      ...
      for (let page = 1; page <= totalPage; page++) {
        ...
        for (const item of pageItems) {
             ...
             if (detail) {
                 ...
                 if (needsTruncate) {
                   addLog("API 테스트 성공. 기존 데이터 초기화 중...", "info", page);
                          ...
                          <div className="flex items-center gap-6">
                            ...
                            {collectionStats.lastPayment && (
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm">
                                  최신: <span className="font-bold text-[#2a9d8f]">{new Date(collectionStats.lastPayment.paidAt).toLocaleDateString()}</span>
